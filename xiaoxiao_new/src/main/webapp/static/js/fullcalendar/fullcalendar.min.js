...
var Toolbar = /** @class */ (function () {
    ...
    Toolbar.prototype.updateTitle = function (text) {
        if (this.el) {
        	var input = '<input onclick="" style="font-size:18px;" id="myRiQiTitle" type="date" value="' + text + '"/>';
            this.el.find('h2').html(input);
            var longTime = this.calendar.currentDate._i;
            var now;
            if(longTime){
            	now = new Date(this.calendar.currentDate._i); 
            }else{
            	now = new Date(); 
            }
			//格式化日，如果小于9，前面补0  
			...
			var _this = this; 
			//完成赋值  
			$('#myRiQiTitle').val(today); 
            $("#myRiQiTitle").change(function(){
            	if($('#myRiQiTitle').val() != ''){
            		thisDate = $('#myRiQiTitle').val();
            	}else{
            		thisDate = new Date();
            		//格式化日，如果小于9，前面补0  
					...
					//拼装完整日期格式  
					var today = thisDate.getFullYear()+"-"+(month)+"-"+(day) ;
					//完成赋值  
					$('#myRiQiTitle').val(today); 
            	}
            	//$('#calendar').fullCalendar('destroy');
            	//initFullCalendar(thisDate);
            	fanhuichongzai();
			});
            
            //console.log(this);
            //console.log(this.calendar.currentDate._d);
        }
    };
