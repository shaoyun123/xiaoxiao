...
<script type="text/javascript">
	...
	function toDo(event, jsEvent, view){
		...
		if (items[1] == "1") {  // 课程
		$.post("chaxundanmenkecheng_jiaoshi",{"id":items[0],"riqi":items[3],"zhouci":items[2],"kaishijieci":items[4],"jieshujieci":items[5],"jiaoshiid":items[6]},function(data){
			var html="";
			...
			if(data.kecheng.jieshuzhou==null || data.kecheng.jieshuzhou==''){
				html += "上课时间段：第"+data.kecheng.kaishizhou+"周"+'\n';
			}else{
		...
		toContentPage('richeng_huodong_jiaoshi?id='+items[0]+'&zhuangtai='+zhuangtai+'&qufen=1');

		} else if(items[1] == "3"){  // 备忘录
			toContentPage("wodebeiwanglu_jiaoshi?id="+items[0]+"&qufen=1");
		} 
	}
	...
	function getData(start, end, timezone, callback){
		...
		if (mydate.getMinutes() < 10) {
			...
			$.ajax({
				type: "POST",
				url: "chaxunrichengbydate_jiaoshi",
				data: {
					riqi: datetime,
				},
				dataType: 'json',
					...
					if (data) {
						...
						if (data.kechengs != null || data.kechengs != "") {
							for (var i = 0; i < data.kechengs.length; i++) {
								...
								if (time <= jieshu_time) {
									event.push({
										id: data.kechengs[i].id + ",1,"+data.kechengs[i].zhouci+","+datetime+","+data.kechengs[i].kaishijieci+","+data.kechengs[i].jieshujieci+","+data.kechengs[i].jiaoXueLouMing,
