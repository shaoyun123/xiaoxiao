...
<div class="row-fluid">
	<div class="box span12">
		...
		<div class="box-content">
			<form action="" id="forms" method="post" class="form-horizontal">
				<input type="hidden" name="id" value="${kecheng.id}"> <input
					type="hidden" name="shangkeriqi" value="${kecheng.shangkeriqi}">
				...
				<div class="control-group">
					...
					</span> <span style="margin-left: 20px;">周 <c:if
				...
				<div id="tiaoting" style="display: none;" class="control-group">
					<label class="control-label" for="tiaotingriqi">选择调停日期：</label>
					<div class="controls">
						<select style="" id="tiaotingriqi" name="tiaotingriqi">
				...
				<div id="wanshanxinxi" class="control-group" style="display: none;">
					<label class="control-label" for="newriqi">调加课时间：</label>
					<div class="controls">
						<input id="newriqi" name="newriqi" type="text" class="Wdate"
							style="height: 25px"
							onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd'})" /> <span
							style="font-size: 13px; font-weight: bold;">（**调加课时间不得超出教学周范围，如有需要请修改教学周）</span><br>
					</div>
					...
					<div class="controls" style="padding-top: 12px;">
						<span style="font-size: 13px;">节次：</span> <select id="kaishijieci"
							name="kaishijieci" style="width: 50px">
							...
							id="jieshujieci" name="jieshujieci" style="width: 50px">
							<c:forEach items="${jieci}" var="jieci">
								<option value="${jieci.id}"
...
<script type="text/javascript">
	...
	function save() {
		...
		$.ajax({
			type : "POST",
			url : 'savetiaotingkecheng_jiaoshi',
