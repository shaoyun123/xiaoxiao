...
<div class="box span12">
	...
	<div class="box-content">
		<div class="row-fluid">
			<form action="" id="chaxunyijian_fdy"
				name="chaxunyijian_fdy" method="post">
				<div style="width: 100%;">
					...
					<c:forEach items="${yijian}" var="YiJian" varStatus="i">
						<tr style="">
							<td>
								...
								<div style="margin-left: 50px;">
									<a style="text-decoration:none;" onclick="pinglun_gly(${YiJian.yijianid})"><span
										id="${YiJian.yijianid}pinglunliang" style="margin-left: 10px;"
										title="评论" class="icon-comment-alt">&emsp;${YiJian.pinglunliang}</span></a>
									<a style="text-decoration:none;" onclick="dianzan_gly(${YiJian.yijianid})"><span
										id="${YiJian.yijianid}"
								...
								<div style="margin-left: 50px; width: 80%">
									<form action="" class="form-inline" method="post" id="subpinglun">
										<input type="hidden" name="id" value="${YiJian.yijianid}">
										<input id="pinglunneirong${YiJian.yijianid}"
											name="pinglunneirong" size="100" type="text" />&emsp;
										<input type="button" value="发送"
											onclick="subpinglun(${YiJian.yijianid})">
									</form>
								</div> <br>
										...
										<c:forEach items="${YiJian.yijianhuifus}" var="yijianhuifu"
											...
											<div id="${yijianhuifu.huifuid}">
												<c:if test="${yijianhuifu.xueshengid!=null}">
													...
													<a style="text-decoration:none;" onclick="huifu(${i.index},${status.index})"><span
														style="float: right; margin-right: 20px;" title="回复${yijianhuifu.xueshengxingming}"
													...
													<c:choose>
														<c:when test="${yijianhuifu.jiaoshiid==userid}">
															...
															<a style="text-decoration:none;"
																onclick="delhuifu_gly(${yijianhuifu.huifuid},${YiJian.yijianid})"><span
														...
														<c:otherwise>
															...
															<a style="text-decoration:none;" onclick="huifu(${i.index},${status.index})"><span
																style="float: right; margin-right: 20px;" title="回复${yijianhuifu.jiaoshixingming}"
																class="icon-comment-alt"></span></a>
														</c:otherwise>
													</c:choose>
												</c:if>
												...
												<c:if test="${yijianhuifu.jiaoshiid!=null}">
													<div id="${i.index}-${status.index}"
														...
														<form action=""  id="subhuifu2" class="form-inline" method="post">
														    <input type="hidden" name="id" id="8${i.index}${status.index}" value="${YiJian.yijianid}_${yijianhuifu.jiaoshiid}">
...
<%-- 																placeholder="回复&emsp;${yijianhuifu.jiaoshixingming}" />&emsp; --%>
															<input type="button" value="发送"
																onclick="subhuifu2(${i.index},${status.index})"><br>
															<br>
														</form>
													</div>
												</c:if>
												<c:if test="${yijianhuifu.xueshengid!=null}">
													<div id="${i.index}-${status.index}"
														style="margin-left: 30px; display: none;">
														<form action="" id="subhuifu" class="form-inline" method="post">
															<input type="hidden" name="id" id="6${i.index}${status.index}" value="${YiJian.yijianid}_${yijianhuifu.xueshengid}">
															<input id="huifuneirong${i.index}${status.index}" name="huifuneirong" size="80" type="text" />&emsp;
<%-- 																placeholder="回复&emsp;${yijianhuifu.xueshengxingming}" />&emsp; --%>
															<input type="button" value="发送"
																onclick="subhuifu1(${i.index},${status.index})"><br>
															<br>
														</form>
													</div>
												</c:if>
			...
			<div class="pagination pagination-centered">
				<ul class="pagination">
					<c:if test="${page >1}">
						<li><a
							href="javascript:void(0);" onclick="toContentPage('chaxunyijian_gly?biaoqian=${biaoqian}&niming=${niming}&page=1')">首页</a></li>
					</c:if>
					<c:if test="${page > 1}">
						<li><a
							href="javascript:void(0);" onclick="toContentPage('chaxunyijian_gly?biaoqian=${biaoqian}&niming=${niming}&page=${page-1}')">上一页</a></li>
					</c:if>
					<li><a href="#">第${page}页</a></li>
					<c:if test="${page < pages}">
						<li><a
							href="javascript:void(0);" onclick="toContentPage('chaxunyijian_gly?biaoqian=${biaoqian}&niming=${niming}&page=${page+1}')">下一页</a></li>
					</c:if>
					<c:if test="${page < pages}">
						<li><a
							href="javascript:void(0);" onclick="toContentPage('chaxunyijian_gly?biaoqian=${biaoqian}&niming=${niming}&page=${pages}')">尾页</a></li>
					</c:if>
				</ul>
			</div>
		</div>
	...
	function subpinglun(id){
		...
		$.ajax({
			...
			success : function(data) {
				...
				toContentPage('yijiandongtai_gly');
			},
			error : function() {
				alert("超时!")
			}
...
// 		alert($("#6"+id1+id2).val() + ":"+$("#huifuneirong"+id1+id2).val());
		var id = $("#6"+id1+id2).val();
		$.ajax({
			type : "POST",
			url : 'subhuifuxuesheng_gly',
			data : {
				id : id,
	...
	function chaqun(){
		...
		toContentPage('chaxunyijian_gly?biaoqian='+biaoqian+'&niming='+niming);
	}
	</script>