...
<div class="box span12">
	...
	<div class="box-content">
		<form action="" id="chaxunyijian_shuji" name="chaxunyijian_shuji"
			method="post">
			<div style="width: 100%;">
				<span style="margin-left: 20px;font-weight: bold">标签:</span>&emsp;
				...
				</select> <span style="margin-left: 20px; font-weight: bold">班级:</span>&emsp;
				...
				</select> <span style="margin-left: 20px; font-weight: bold">匿名状态:</span>&emsp;
				...
				<button type="button" style="margin-left: 50px; width: 5%" class="btn btn-success"
					...
					<c:forEach items="${yijian}" var="YiJian" varStatus="i">
						<tr style="background-color: #ffffff;">
							<td>
								...
								<br>
								<div style="margin-left: 50px;">
									<a style="text-decoration:none;" onclick="pinglun_fdy('${YiJian.yijianid}')"><span
										id="${YiJian.yijianid}pinglunliang" style="margin-left: 10px;"
										title="评论" class="icon-comment-alt">&emsp;${YiJian.pinglunliang}</span></a>
									<a style="text-decoration:none;" onclick="dianzan_shuji(${YiJian.yijianid})"><span
										id="${YiJian.yijianid}"
											...
											<div id="${yijianhuifu.huifuid}">
												<c:if test="${yijianhuifu.xueshengid!=null}">
													...
													<a style="text-decoration:none;" onclick="huifu(${i.index},${status.index})"><span
														...
														title="回复${yijianhuifu.xueshengxingming}"
													...
													<c:choose>
														<c:when test="${yijianhuifu.jiaoshiid==userid}">
															...
															<a style="text-decoration:none;"
																onclick="delhuifu_shuji(${yijianhuifu.huifuid},${YiJian.yijianid})"><span
										...
										<c:choose>
												...
												<c:if test="${yijianhuifu.jiaoshiid!=null}">
													<div id="${i.index}-${status.index}"
														...
														<form action="" class="form-inline" method="post"
															id="form2">
															<input type="hidden" id="8${i.index}${status.index}"
																name="id"
																value="${YiJian.yijianid}_${yijianhuifu.jiaoshiid}" /> <input
																id="huifuneirong${i.index}${status.index}"
																name="huifuneirong" size="80" type="text" />&emsp;
<%-- 																placeholder="回复&emsp;${yijianhuifu.jiaoshixingming}" />&emsp; --%>
															<button type="button" 
																onclick="subhuifu2(${i.index},${status.index})">发送</button><br>
															<br>
														</form>
													</div>
												</c:if>
												<c:if test="${yijianhuifu.xueshengid!=null}">
													<div id="${i.index}-${status.index}"
														...
														<form action="" class="form-inline" method="post"
															id="form1">
															<input type="hidden" id="6${i.index}${status.index}"
																name="id"
...
<%-- 																placeholder="回复&emsp;${yijianhuifu.xueshengxingming}" />&emsp; --%>
															<button type="button" 
																onclick="subhuifu1(${i.index},${status.index})">发送</button><br>
															<br>
														</form>
													</div>
												</c:if>
			...
			<!-- 			<ul class="pagination"> -->
			<%-- 				<c:if test="${page >1}"> --%>
			<%-- 					<li><a href="chaxunyijian_shuji?banji=${chaxunbanjiid}&biaoqian=${biaoqian}&niming=${niming}&page=1">首页</a></li> --%>
			<%-- 				</c:if> --%>
			<%-- 				<c:if test="${page > 1}"> --%>
			<%-- 					<li><a href="chaxunyijian_shuji?banji=${chaxunbanjiid}&biaoqian=${biaoqian}&niming=${niming}&page=${page-1}">上一页</a></li> --%>
			<%-- 				</c:if> --%>
			<%-- 				<li><a href="#">第${page}页</a></li> --%>
			<%-- 				<c:if test="${page < pages}"> --%>
			<%-- 					<li><a href="chaxunyijian_shuji?banji=${chaxunbanjiid}&biaoqian=${biaoqian}&niming=${niming}&page=${page+1}">下一页</a></li> --%>
			<%-- 				</c:if> --%>
			<%-- 				<c:if test="${page < pages}"> --%>
			<%-- 					<li><a href="chaxunyijian_shuji?banji=${chaxunbanjiid}&biaoqian=${biaoqian}&niming=${niming}&page=${pages}">尾页</a></li> --%>
			<%-- 				</c:if> --%>
			<!-- 			</ul> -->
			<!-- 		</div> -->
		</div>
...
// 		console.log(chulibiaoji);
		console.log(niming);
		toContentPage("chaxunyijian_shuji?biaoqian="+biaoqian+"&banji="+banji+"&niming="+niming);
	}
	
	function dianzan_shuji(id){
		var code=id;
		$.ajax({
			type:"POST",
			url:"dianzan_shuji",
		...
		if($("#pinglunneirong"+id).val()==""){
			...
			$.ajax({
				...
				success : function(data) {
					...
					toContentPage('yijiandongtai_shuiji');
				},
				error : function() {
					alert("超时!");
					toContentPage('yijiandongtai_shuiji');
		...
		if($("#huifuneirong"+id1+id2).val()==""){
			...
			$.ajax({
				...
				success : function(data) {
					...
					toContentPage('yijiandongtai_shuji');
				},
				error : function() {
					alert("超时!");
					toContentPage('yijiandongtai_shuji');
	...
	function delhuifu_shuji(id1,id2){
		var code=id1;
