										...
										<tbody>
											<c:forEach items="${renyuans}" var="renyuan">
												<tr height=65px>
													...
													<td>${renyuan.zhiwu}</td>
													<c:choose>
														<c:when test="${!fn:contains(renyuan.zhiwu,'编辑')}">
															<td><input type="button" class="btn btn-default" value="设为编辑" onclick="Server(${renyuan.xueshengid })"></td>
														</c:when>
														<c:otherwise>
															<td><input type="button" class="btn btn-default" value="设为编辑" disabled="disabled"></td>
														</c:otherwise>
													</c:choose>
												</tr>
											</c:forEach>
		...
		<div>
			...
			function Server(id){
				if(cfm()){
					...
					var code = id+",zytech,"+xueshengzuzhixinxiid;
					$.ajax({
						type: "POST",
						url: 'xsdbj',
				    	data:{CODE:code},
						...
						success:function(data)
						{
							if(data=="success"){
								alert("成功！")
								window.location.href="xzdbj?id=${xueshengzuzhixinxiid}";
							}
							else{
								alert("fail")
							}
