...
<body class="flat-blue">
	<div class="app-container">
										...
										<tbody>
											<c:forEach items="${renyuans}" var="renyuan">
												<tr height=65px>
													...
													<td>${renyuan.zhiwu}</td>
													<c:choose>
														<c:when test="${fn:contains(renyuan.zhiwu,'负责人')}">
															<td><input type="button" class="btn btn-default" value="设为负责人" disabled="disabled"></td>
														</c:when>
														<c:otherwise>
															<td><a href="xzdfzr?id=${renyuan.xueshengid }&&xsxx=${xueshengzuzhixinxiid}" onclick="return cfm()"><input type="button" class="btn btn-default" value="设为社长"></a></td>
														</c:otherwise>
													</c:choose>
												</tr>
											</c:forEach>
		...
		<div>
			...
			<script type="text/javascript">
			function cfm() {
			if (confirm("确认设定？设定之后你将失去负责人权限！") == true) {
				return true;
			} else {
				return false;
			}
