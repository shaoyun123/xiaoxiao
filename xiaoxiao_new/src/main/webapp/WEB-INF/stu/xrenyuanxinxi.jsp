	...
	<div class="app-container">
						...
						<div class="col-xs-12">
							<div class="card">
								<div class="card-body">
											...
											<c:forEach items="${renyuans}" var="renyuan">
												<tr height=65px>
													...
													<td><c:choose>
														...
														<select id="${renyuan.xueshengid }" name="${renyuan.xueshengid }" class="form-control chosen-select success" style="width:50%" aria-required="true" aria-invalid="false">
															<option value="">请选择</option>
															<c:forEach items="${bumen}" var="bumen">
															<c:if test="${bumen.mingcheng!=renyuan.bumen}">
														...
														<c:otherwise>
															<c:choose>
																<c:when test="${!fn:contains(renyuan.zhiwu,'部长')}">
																	<td><input type="button" class="btn btn-default" value="移入部门" onclick="Server(${renyuan.xueshengid })"></td>
																</c:when>
																<c:otherwise>
																	<td><input type="button" class="btn btn-default" value="移入部门" disabled="disabled"></td>
																</c:otherwise>
															</c:choose>
														</c:otherwise>
													</c:choose>
									...
									<input type="hidden" name="id" id="id" value="${xueshengzuzhixinxiid}">
								</div>
							</div>
						</div>
					</div>
		...
		<div>
			...
			function Server(id){
				...
				if(cfm(option.text())){
					var xueshengzuzhixinxiid = $("#id").val();
					var code = id+",zytech,"+xueshengzuzhixinxiid+",zytech,"+option.val();
					$.ajax({
						type: "POST",
						url: 'xyrbm',
				    	data:{CODE:code},
						...
						success:function(data)
						{
							if(data=="success"){
								alert("成功！")
								window.location.href="xcksy?id=${xueshengzuzhixinxiid}";
							}
							else{
								alert("fail")
							}
