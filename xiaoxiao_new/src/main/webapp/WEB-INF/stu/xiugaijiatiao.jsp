...
.box {
	border: solid 1px #ccc;
		...
		<div class="box-header">
			<h2><i class="icon-align-justify"></i><span class="break"></span>修改假条</h2>
		</div>
					<div class="box-content">
								...
								<div class="card-body">
									<form id="application"
										action="SubXiuGai?id=${jiatiao.qingjiaid}" class="form-inline"
												...
												<div id = "imgAll" class="imgAll">
													<ul id="imgul">
														<c:forEach items="${jiatiao.tupian }" var="tupian">
															<li><img src='getPic?id=${tupian}' id = "${tupian }" class='imsg'><i class='delImg'>X</i></li>
														</c:forEach>
													</ul>
												</div>
											</div>
										</div>
										<input type="hidden" id = "src" name="src" value="${jiatiao.bingjiazhengming }">
										<div class="sub-title">
											<label>时间：</label> 自：<input type="text" id="kaishishijian" class="Wdate" style="height:25px"
												name="kaishishijian" value="${jiatiao.qingjiakaishishijian}" onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd'})">
											至：<input type="text" id="jieshushijian"  class="Wdate" style="height:25px" name="jieshushijian" onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd'})" 
	...
	$("#uploadDiv").on("click",function(){
		 var li =document.getElementById("imgul").getElementsByTagName("li").length;
	     ...
	     $("#uploaderInput").bind("change",function(e){
	    	 ...
	    	 document.getElementById("imgul").appendChild(dom);
	    	 $("#"+li).attr("src",URL.createObjectURL($(this)[0].files[0]));
	    	 $(this).removeAttr("id");
	    	 $(this).attr('id',li+"input");
	     });
	     $("#uploaderInput").click();
	 });  	
