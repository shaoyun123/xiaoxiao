		...
		<div class="box-content">
			<table class="table table-bordered table-striped table-condensed bootstrap-datatable">
				<c:forEach items="${tianxietiaomu}" var="tiaomu">
					...
					<tr>
						<th
							style="font-size: 25px; padding-left: 0em; width: 30%; text-align: left;">${tiaomu.mingcheng}(${tiaomu.manfen })</th>
						<td></td>
					</tr>
					<c:forEach items="${ChildList}" var="childlist">
						...
						<tr>
							<td
								style="font-size: 20px; width: 30%; padding-left: 2em; text-align: left;"><strong>${childlist.mingcheng}(${childlist.manfen})</strong></td>
							<c:choose>
						...
						<c:forEach items="${List}" var="list">
							<tr>
								<td
									style="font-size: 15px; padding-left: 4em; text-align: left;">${list.mingcheng}(${list.manfen})</td>
								<td style="font-size: 15px; text-align: center;"><c:choose>
...
<!-- 	<button type="button" class="btn btn-default"  onclick="goback()">返回上一页</button> -->
<script type="text/javascript">
			...
			function goback(){
				window.location.href="fabukaoping";
				return false;
			}
			</script>
