...
public class AppShiJianKeKaoPingController {
	...
	public JSONObject app_isfenzu(HttpServletRequest request, HttpServletResponse response) throws ParseException {
		...
		if (str.equals(token)) {
			...
			if (shiJianKeCheng.get("xiaoZuRongLiang") != null) {
				String xiaozurongliang = shiJianKeCheng.get("xiaoZuRongLiang").toString().split("_")[1];
				String fenxiaozu = shiJianKeCheng.get("xiaoZuRongLiang").toString().split("_")[0];
				String fendazu = shiJianKeCheng.get("xiaoZuRongLiang").toString().split("_")[2];
				if (fendazu.equals("0")) {
				} else {
		...
		if (str.equals(token)) {
			...
			for (int j = 0; j < xueXiZus.size(); j++) {
				if (xueXiZus.get(j).get("chengYuanIDLieBiao").toString() != null) {
					String[] chengyuan = xueXiZus.get(j).get("chengYuanIDLieBiao").toString().substring(1).split(",");
					List<Map<String, Object>> member = new ArrayList<>();
					for (int i = 0; i < chengyuan.length; i++) {
						Map<String, Object> map = new HashMap<>();
						XueSheng xSheng = xueShengService.selectByPrimaryKey(Integer.parseInt(chengyuan[i]));
						map.put("xingming", xSheng.getXingming());
						map.put("xueshengid", xSheng.getXueshengid());
						member.add(map);
					}
					xueXiZus.get(j).put("chengyuan", member);
					xueXiZus.get(j).put("xiaozuzhang",
							xueShengService
									.selectByPrimaryKey(Integer.parseInt(xueXiZus.get(j).get("xueShengID").toString()))
									.getXingming());
				}
			}
	...
	public JSONObject app_getzuxinxi(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (str.equals(token)) {
			...
			for (int i = 0; i < daZuList.size(); i++) {
				...
				// String xueShengIDs = "";
				// for (int m = 0; m < xiaozus.size(); m++) {
				// // 取得小组内学生的ID
				// xueShengIDs +=
				// xiaozus.get(i).get("chengYuanIDLieBiao").toString().substring(1);
				// }
				...
				// listXueSheng =
				// xueShengService.selectByXueShengIDs(Arrays.asList(xueShengIDs.split(",")));
	...
	private Map<String, Object> getBenZuXinXi(Integer keID, Integer kaoPingID, Integer xueshengid, String fendazu,
		...
		if (chouqian == null) {
			...
			if (chouqian.getQianDao() != null) {
				if (chouqian.getQianDao().toString().indexOf("," + xueXiZus.get(0).getId() + ",") >= 0) {
	...
	private Map<String, Object> getDangQianAndNextZu(Integer kechengid, Integer kaopingid, Integer xueshengid,
		...
		if (fendazu.equals("0")) {
			...
			map.put("dazuzhangid", xueXiZus.get(0).getFenZuId());
		}
			...
			if (app_KaoPingChouQian.getChouQian() != null && !"".equals(app_KaoPingChouQian.getChouQian())) {
				...
				if (app_XueXiZu3 != null && app_XueXiZu3.size() > 0) {
					...
					if (app_XueXiZu3.get(0).getChengYuanLieBiao().toString() != null) {
						String[] aMembers = app_XueXiZu3.get(0).getChengYuanLieBiao().toString().substring(1)
								.split(",");
						List<Object> list2 = new ArrayList<>();
						if (aMembers != null && aMembers.length > 0) {
							for (int j = 0; j < aMembers.length; j++) {
	...
	public JSONObject app_dafen(HttpServletRequest request, HttpServletResponse response) {
		...
		case "0":
			dazuid = null;
			jianChaDaZuDaFen(kaopingid, dazuid);
			break;
		case "1":
			// 分大组打分，检查本大组的情况
			jianChaDaZuDaFen(kaopingid, dazuid);
	 ...
	 * 
	 */
	private void jianChaDaZuDaFen(String kaoPingID, String daZuID) {
		// 获取本次考评的签到信息
		...
		mapsss.put("dazuid", daZuID);
		List<App_KaoPingChouQian> chouQians = app_KaoPingChouQianService.getchouqian(mapsss);
