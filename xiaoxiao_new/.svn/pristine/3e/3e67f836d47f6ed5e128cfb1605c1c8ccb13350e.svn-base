...
public interface KeChengService {
	...
	List<KeCheng> getAllBybanJiID(String banjiid1,String banjiid2);//get符合banjiid的课程,一个班级两个位置1*2
	
	List<KeCheng> getAllBybanJiIDs(String banjiids);//get符合banjiids的课程，多个班级
	
	List<KeCheng> getAllByxueShengID(String xueshengid1,String xueshengid2);
	...
	int updatecanYuRenByPrimaryKey(KeCheng record);//学生选择加课
	
	int updateshangKeRiQiByPrimaryKey(KeCheng record);//调停课
	
	 int updateByKeDaiBiao(KeCheng record);//设置课代表
	...
	List<KeCheng> getAllByShangKeJiaoShiAndXuenianAndXueqi(Map<String, String> map2);
	...
	List<Map<String, Object>> getShangKeShiJianByID(String string);
	...
	List<Map<String, Object>> getKeChengByLaoShiID(Map<String, String> paramMap);
