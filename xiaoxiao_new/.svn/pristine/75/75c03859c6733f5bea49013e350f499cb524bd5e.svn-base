...
// 			window.location.href="chaxundanmenkecheng?id="+items[0];
		$.post("chaxundanmenkecheng",{"id":items[0],"riqi":items[3],"zhouci":items[2],"kaishijieci":items[4],"jieshujieci":items[5],"jiaoshiid":items[6]},function(data){
			var html="";
		...
		toContentPage('richeng_huodong_fdy?id='+items[0]+'&zhuangtai='+zhuangtai+'&qufen=1');

		} else if(items[1] == "3"){  // 备忘录
			toContentPage('mybeiwanglu_fdy?id='+items[0]+'&qufen=1');
			
			
		} else if(items[1] == "4"){  // 查寝
			toContentPage('chaqinanpai?id='+items[0]);
		}
	}
