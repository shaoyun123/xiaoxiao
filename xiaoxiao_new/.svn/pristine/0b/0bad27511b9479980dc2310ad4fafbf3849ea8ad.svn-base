				...
				<div class="container-fluid">
					<div class="navbar-header">
						...
						<ol class="breadcrumb navbar-breadcrumb">
							...
							<li class="active">增删部门</li>
							
						</ol>
						<a href="xaddbumen?id=${xueshengzuzhixinxiid}" ><span title="新增部门" class="glyphicon glyphicon-plus" style="padding-top:20px;padding-left:20px;font-size:18px"></span></a>
						<button type="button"
										...
										<tbody>
											<c:forEach items="${bumen}" var="bumen">
												<tr height=65px>
													...
													<td>${bumen.chuangjianshijian}</td>
													<td><input type="button" class="btn btn-default" value="删除" onclick="Server(${bumen.bumenid})"></td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
		...
		<div>
			...
			function Server(id){
				if(cfm()){
					...
					var code =xueshengzuzhixinxiid+",zytech,"+id;
					$.ajax({
						type: "POST",
						url: 'xdelbumen',
				    	data:{CODE:code},
						...
						success:function(data)
						{
							if(data=="success"){
								alert("成功！")
								window.location.href="xzsbm?id=${xueshengzuzhixinxiid}";
							}
							else{
								alert("fail")
							}
