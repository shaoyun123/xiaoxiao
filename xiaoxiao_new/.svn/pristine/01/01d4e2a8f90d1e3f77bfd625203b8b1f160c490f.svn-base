	...
	<div class="box span12">
		...
		<div class="box-content">
			<div class="card-body">
				<form
					action=""
					class="form-horizontal" method="post" id="forms">
					<fieldset>
					...
					<div class="control-group">
						...
						<div class="controls">
							<input id="mingcheng" name="mingcheng" type="text"
								value="${huodong.huodongmingcheng}" class="input-xlarge focused"
								size="30" />
						</div>
					</div>
					<div class="control-group">
						...
						<div class="controls">
							<input id="didian" name="didian" type="text"
								value="${huodong.didian}" class="input-xlarge focused" size="30" />
						</div>
					</div>
					<div class="control-group">
						...
						<div class="controls">
							<input id="beizhu" name="shuoming" type="text"
								value="${huodong.shuoming}" class="input-xlarge focused"
								size="30" />
						</div>
					</div>
					<!-- 					<div class="sub-title"> -->
					<!-- 						<span style="font-weight: bold;">参与人：</span>&emsp;<input -->
					<!-- 							id="Button1" type="button" name="btn1" value="按学生添加" -->
					<!-- 							onclick="ShowDiv('MyDiv')" /> &emsp;<input id="Button2" -->
					<!-- 							type="button" name="btn2" value="按班级添加" -->
					<!-- 							onclick="ShowDiv('MyDiv2')" /> <input id="canyuren" -->
					<!-- 							name="canyuren" type="hidden" /> <input id="huodongbanji" -->
					<!-- 							name="huodongbanji" type="hidden" /> <input type="hidden" -->
					<!-- 							name="leixing" id="leixing"> <input type="hidden" -->
					<!-- 							name="leixing2" id="leixing2"> -->
					<!-- 					</div> -->
					...
					<div class="control-group">
						...
						<div class="controls">
							...
							</label> <input id="canyuren" name="canyuren" type="hidden" /> <input
...
<!-- 									</div> -->
						...
						<div class="control-group">
							...
							<div class="controls">
								<input id="riqi" name="riqi" type="text" class="Wdate" value="${huodong.riqi}"
									style="height: 25px"
						...
						<div class="control-group">
							...
							<div class="controls">
								<input id="kaishishijian" name="kaishishijian" type="text" value="${huodong.kaishishijian}" 
						...
						<div class="control-group">
							...
							<div class="controls">
								<input id="jieshushijian" name="jieshushijian" type="text" value="${huodong.jieshushijian}" 
									class="Wdate" style="height: 25px"
									onclick="WdatePicker({readOnly:true,dateFmt:'HH:mm'})" />
							</div>
						</div>
						<div class="control-group">
							<c:choose>
								<c:when test="${not empty beiwl}">
<!-- 									<span style="font-weight: bold;"><input type="checkbox" -->
<!-- 										id="check1" onclick="check()" checked="checked" value="1">预提醒时间：</span> -->
<!-- 									<div id="hidetime" style="display: block"> -->
<!-- 										<select id="booktime" name="booktime"> -->
<!-- 											<option value="">--请选择--</option> -->
<!-- 											<option value="2" -->
<%-- 												<c:if test="${prehour==2}">selected="selected"</c:if>>提前2小时</option> --%>
<!-- 											<option value="4" -->
<%-- 												<c:if test="${prehour==4}">selected="selected"</c:if>>提前4小时</option> --%>
<!-- 											<option value="6" -->
<%-- 												<c:if test="${prehour==6}">selected="selected"</c:if>>提前6小时</option> --%>
<!-- 											<option value="8" -->
<%-- 												<c:if test="${prehour==8}">selected="selected"</c:if>>提前8小时</option> --%>
<!-- 											<option value="24" -->
<%-- 												<c:if test="${prehour==24}">selected="selected"</c:if>>提前一天</option> --%>
<!-- 										</select> -->
<!-- 									</div> -->
									<label class="control-label" for="jieshushijian">预提醒时间：</label>
							<div class="controls">
								<label class="checkbox inline"> <input type="checkbox"
									id="check1" onclick="check()" checked="checked" value="1">
								
								<div id="hidetime" style="display: block" style="width:120px;">
									<select id="booktime" name="booktime">
										<option value="">--请选择--</option>
											<option value="2"
												<c:if test="${prehour==2}">selected="selected"</c:if>>提前2小时</option>
								...
								<c:otherwise>
<!-- 									<span style="font-weight: bold;"><input type="checkbox" -->
<!-- 										id="check1" onclick="check()" value="0">预提醒时间：</span> -->
...
<!-- 									</div> -->
							...
							<div class="controls">
								<label class="checkbox inline"> <input type="checkbox"
									id="check1" onclick="check()" value="0">
						...
						<c:forEach items="${xueshengs }" var="xs">
							<div id="ax">
								<div class="op">
									<input name="canyurenbanjiid" id="canyurenbanjiid"
										type="hidden" value="${xs.banjiid}" /> <input
										name="canyurenxueshengid" id="canyurenxueshengid"
										type="hidden" value="${xs.xueshengid}" /> <input
										name="canyurenxuehao" id="canyurenxuehao" type="hidden"
										value="${xs.xuehao}" /> <input name="canyurenbanjimingcheng"
										id="canyurenbanjimingcheng" type="hidden"
										value="${xs.banjimingcheng}" /> <input
										name="canyurenxingming" id="canyurenxingming" type="hidden"
										value="${xs.xingming}" />
								</div>
							</div>
						</c:forEach>
						<c:forEach items="${huodongbanji}" var="hb">
							<div id="bx">
								<div class="se">
									<input name="huodongbanjiid" id="huodongbanjiid" type="hidden"
										value="${hb.banjiid}" /> <input name="huodongbanjimingcheng"
										id="huodongbanjimingcheng" type="hidden"
										value="${hb.banjimingcheng}" />
								</div>
							</div>
						</c:forEach>
						</fieldset>
...
$(function() {
	...
	function ShowDiv(show_div) {
		if (show_div == "MyDiv") {
			...
			document.getElementById("MyDiv2").style.display = 'none';
			var num = $("#ax .op").length;
			var canyurenbanjiids = document
					.getElementsByName("canyurenbanjiid");
			var canyurenxueshengids = document
					.getElementsByName("canyurenxueshengid");
			var canyurenxuehaos = document.getElementsByName("canyurenxuehao");
			var canyurenxingmings = document
					.getElementsByName("canyurenxingming");
			var canyurenbanjimingchengs = document
					.getElementsByName("canyurenbanjimingcheng");
			// 							alert(num);
			var a = document.getElementById("beixuanren").getElementsByTagName(
					"tr");
			// 							alert(a.length);
			if (a.length < num) {
				for (var i = 0; i < num; i++) {
					var code = "";
					code += '<tr id="'+canyurenxueshengids[i].value+'"><td>'
							+ canyurenbanjimingchengs[i].value
							+ '</td><td>'
							+ canyurenxuehaos[i].value
							+ '</td><td>'
							+ canyurenxingmings[i].value
							+ '</td><td><span style="color:blue;cursor:pointer;"onclick="del(0,\''
							+ canyurenxueshengids[i].value
							+ '\')"">删除</span></td></tr>';

					$("#beixuanren").append(code);
					id = canyurenxueshengids[i].value
							+ canyurenxueshengids[i].value;
					console.log(id);
		...
		if (show_div == "MyDiv2") {
			...
			document.getElementById(show_div).style.display = 'block';
			document.getElementById("MyDiv").style.display = 'none';
			var num = $("#bx .se").length;
			var huodongbanjimingcheng = document
					.getElementsByName("huodongbanjimingcheng");
			var huodongbanjiid = document.getElementsByName("huodongbanjiid");
			var a = document.getElementById("beixuanren2")
					.getElementsByTagName("tr");
			if (a.length < num) {
				for (var i = 0; i < num; i++) {
					var code = "";
					code += '<tr id="'+huodongbanjiid[i].value+'"><td>'
							+ huodongbanjimingcheng[i].value
							+ '</td><td><span style="color:blue;cursor:pointer;" onclick="del2(\''
							+ huodongbanjiid[i].value
							+ '\')">删除</span></td></tr>';
					$("#beixuanren2").append(code);
					id = huodongbanjiid[i].value + '' + huodongbanjiid[i].value;
					console.log(id);
					document.getElementById(id).style.display = "none";
