...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
	...
	<div class="box span12">
		<div class="box-header">
			<h2><i class="icon-align-justify"></i><span class="break"></span>请假已处理</h2>
		</div>
...
<!-- 															type="button" class="btn btn-default" value="详情"></a> -->
          		<button type="button" class="btn btn-success" value="详情" onclick="toContentPage('jiaTiaoDetail_sj?id=${qingjiatiao.qingjiaid}')">详情</button>                            					
									...
									<div class="pagination pagination-centered">
										<ul class="pagination">
											<c:if test="${page >1}">
												<li><a href="qingjiayichuli_sj">首页</a></li>
											</c:if>
											<c:if test="${page > 1}">
												<li><a href="qingjiayichuli_sj?page=${page-1}">上一页</a></li>
											</c:if>
											<li><a href="#">第${page}页</a></li>
											<c:if test="${page < pages}">
												<li><a href="qingjiayichuli_sj?page=${page+1}">下一页</a></li>
											</c:if>
											<c:if test="${page < pages}">
												<li><a href="qingjiayichuli_sj?page=${pages}">尾页</a></li>
											</c:if>
										</ul>
									</div>
								</div>
