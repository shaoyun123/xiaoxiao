...
var fmid = "fhindex";
var mid = "fhindex";
function siMenu(id,fid,MENU_NAME,MENU_URL,MENUID){
	if(id != mid){
		$("#"+mid).removeClass();
		mid = id;
	}
	...
	top.mainFrame.tabAddHandler(id,MENU_NAME,MENU_URL,MENUID);
...
$(function(){
	$.ajax({
		type: "POST",
		url: locat+'/getUname.do?tm='+new Date().getTime(),
    	data: encodeURI(""),
		dataType:'json',
		...
		success: function(data){
			//alert(data.list.length);
			 $.each(data.list, function(i, list){
				 //登陆者资料
				 $("#user_info").html('<small>Welcome</small> '+list.SNAME+'');
				 user = list.SNAME;
				 USER_ID = list.SID;//用户ID
				 hf(list.SSKIN);//皮肤
				 if(list.SID != '-1'){
