...
<script type="text/javascript">
	...
	function toDo(event, jsEvent, view) {
		...
		// 			window.location.href="chaxundanmenkecheng?id="+items[0];
			$.post("chaxundanmenkecheng_gly", {
				"id" : items[0]
			}, function(data) {
				var html = "";
				html += "课程名字：" + data.kecheng.kechengmingcheng + '\n';
				html += "任课教师：" + data.kecheng.renkejiaoshi + '\n';
				html += "上课时间：" + data.kecheng.kaishishijian + " -- "
						+ data.kecheng.jieshushijian + '\n';
				html += "上课校区：" + data.kecheng.xiaoquming + '\n';
				html += "上课地点：" + data.kecheng.jiaoshiming + '\n';
				html += "上课时间段：第" + data.kecheng.kaishizhou + "-"
						+ data.kecheng.jieshuzhou + "周" + '\n';
				alert(html);
			...
			toContentPage('richeng_huodong_gly?id=' + items[0]
					+ '&zhuangtai=' + zhuangtai);
		} else if (items[1] == "3") { // 备忘录
			toContentPage('wodebeiwanglu_gly?id=' + items[0]
					+ '&qufen=1');
		}
	}
						...
						if (data) {
							...
							if (data.kechengs != null || data.kechengs != "") {
								for (var i = 0; i < data.kechengs.length; i++) {
									var jieshu_time = datetime + ' '
											+ data.kechengs[i].jieshushijian;
									if (time <= jieshu_time) {
										event
												.push({
													id : data.kechengs[i].id
															+ ",1",
													title : "地点："
															+ data.kechengs[i].xiaoquming
															+ " "
															+ data.kechengs[i].jiaoshiming
															+ "@-@课程："
															+ data.kechengs[i].kechengmingcheng,
													start : datetime
															+ " "
															+ data.kechengs[i].kaishishijian,
													color : '#32CD32;',
													end : datetime
															+ " "
															+ data.kechengs[i].jieshushijian,
							...
							if (data.huodongs != null || data.huodongs != "") {
								for (var j = 0; j < data.huodongs.length; j++) {
									var jieshu_time = datetime + ' '
											+ data.huodongs[j].jieshushijian;
									if (time <= jieshu_time) {
										if (data.huodongs[j].zhuangtai == "0") {
											event
													.push({
														id : data.huodongs[j].huodongid
																+ ",2",
														title : "地点："
																+ data.huodongs[j].didian
																+ "@-@待确认活动："
																+ data.huodongs[j].huodongmingcheng,
														start : datetime
																+ " "
																+ data.huodongs[j].kaishishijian,
														color : '#EEAD0E;',
														end : datetime
																+ " "
																+ data.huodongs[j].jieshushijian,
							...
							if (data.beiwanglus != null
									&& data.beiwanglus != "") {
								for (var k = 0; k < data.beiwanglus.length; k++) {
									if (time <= data.beiwanglus[k].shijian) {
										event
												.push({
													id : data.beiwanglus[k].id
															+ ",3",
													title : "地点："
															+ data.beiwanglus[k].didian
															+ "@-@事件："
															+ data.beiwanglus[k].neirong,
